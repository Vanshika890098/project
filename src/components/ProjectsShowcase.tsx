...
export default function ProjectsShowcase() {
  ...
  const section = data.find((sec: any) => sec.__component === 'sections.projects-showcase-section');
  const carouselRef = useRef<HTMLDivElement>(null);

  const baseUrl = "https://slather-student-refresh.ngrok-free.dev";
  const heading = section?.heading || "Have a view of our amazing projects with our clients";
  const badgeText = section?.badgeText || "Portfolio";
  
  const defaultProjects = [
    { id: '1', heading: 'Financial App', sidetext: 'Mobile Dev', image: { url: 'https://images.unsplash.com/photo-1551288049-bebda4e38f71?auto=format&fit=crop&q=80&w=800' } },
    { id: '2', heading: 'Data Dashboard', sidetext: 'Web App', image: { url: 'https://images.unsplash.com/photo-1460925895917-afdab827c52f?auto=format&fit=crop&q=80&w=800' } },
    { id: '3', heading: 'VR Experience', sidetext: 'AR/VR', image: { url: 'https://images.unsplash.com/photo-1593508512255-86ab42a8e620?auto=format&fit=crop&q=80&w=800' } },
    { id: '4', heading: 'E-commerce Platform', sidetext: 'Web Dev', image: { url: 'https://images.unsplash.com/photo-1472851294608-062f824d29cc?auto=format&fit=crop&q=80&w=800' } }
  ];

  const projects = section?.clientdetails?.length > 0 ? section.clientdetails : defaultProjects;
      ...
      <div className="container mx-auto px-6 md:px-12">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
          ...
          <div className="flex gap-4 shrink-0 pb-2">
            <button 
              onClick={scrollLeft}
              className="w-12 h-12 rounded-full border border-gray-200 flex items-center justify-center text-gray-500 hover:bg-primary hover:text-white hover:border-primary transition-all cursor-pointer"
            >
            ...
            <button 
              onClick={scrollRight}
              className="w-12 h-12 rounded-full bg-primary text-white flex items-center justify-center hover:bg-secondary transition-all shadow-lg shadow-primary/30 cursor-pointer"
            >
               <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        >
          {projects?.slice(0, 4).map((project: any) => {
            const imageUrl = project?.image?.url 
              ? (project.image.url.startsWith('http') ? project.image.url : `${baseUrl}${project.image.url}`)
            ...
            return (
              <div key={project.id} className="min-w-[85vw] md:min-w-[450px] shrink-0 snap-start group relative rounded-2xl overflow-hidden h-[500px] cursor-pointer shadow-sm hover:shadow-2xl transition-all">
                 <img src={imageUrl} alt={project.heading} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
