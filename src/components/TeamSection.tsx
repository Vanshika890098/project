...
export default function TeamSection() {
  ...
  const section = data.find((sec: any) => sec.__component === 'sections.team-section');
  const baseUrl = "https://slather-student-refresh.ngrok-free.dev";

  const badgeText = section?.badge_text || "Our Team";
  const heading = section?.heading || "Meet the talented team to drive company";
  ...
  const team = section?.profileimage?.length > 0 ? section.profileimage : defaultTeam;
      ...
      <div className="container mx-auto px-6 md:px-12">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
          {team.slice(0, 4).map((member: any) => {
            let imageUrl = member.imageUrl || "https://images.unsplash.com/photo-1494790108377-be9c29b29330?auto=format&fit=crop&q=80&w=400";
            if (member.image?.[0]?.url) {
              imageUrl = member.image[0].url.startsWith('http') 
                ? member.image[0].url 
            ...
            return (
              <div key={member.id} className="group cursor-pointer">
                <div className="bg-[#ecf3fb] rounded-[1.5rem] p-8 pb-10 transition-all duration-300 hover:shadow-2xl hover:bg-white border border-transparent hover:border-gray-100 hover:-translate-y-2">
                  ...
                  <p className="text-primary font-bold text-sm tracking-wide">{member.text || member.role}</p>
                </div>
              </div>
            );
          })}
